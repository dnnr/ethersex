  ...
  do
  {
    if (ow_sensors[i].ow_rom_code.raw != 0)
    {
           ...
           ow_eeprom(&ow_sensors[i].ow_rom_code)))
      {
        ...
        char temperature[7];
        itoa_fixedpoint(ow_sensors[i].temp.val, ow_sensors[i].temp.digits,
                        temperature);
#endif
...
parse_cmd_onewire_get(char *cmd, char *output, uint16_t len)
{
  ...
  if (ow_temp_sensor(&rom))
  {
    ...
    if (sensor != NULL)
      ret = itoa_fixedpoint(sensor->temp.val, sensor->temp.digits, output);
    else
...
parse_cmd_onewire_get(char *cmd, char *output, uint16_t len)
{
  ...
  if (ow_temp_sensor(&rom))
  {
    ...
    ret = itoa_fixedpoint(temp.val, temp.digits, output);
